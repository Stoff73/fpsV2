<template>
  <div class="max-w-5xl mx-auto">
    <div class="text-centre mb-8">
      ...
      <div class="max-w-3xl mx-auto mb-6 text-left bg-white rounded-lg border border-gray-200 p-6">
        ...
        <ul class="space-y-2 text-body-sm text-gray-600">
          <li class="flex items-start">
            <svg class="w-5 h-5 text-primary-600 mr-2 mt-0.5 flex-shrink-0" fill="none" stroke="currentColour" viewBox="0 0 24 24">
      ...
      <div class="text-centre mt-8">
        <button
          @click="selectFocusArea('estate')"
          :disabled="loading"
          class="inline-flex items-centre px-8 py-3 border border-transparent text-body font-medium rounded-button text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colours"
        >
          <svg v-if="!loading" class="w-5 h-5 mr-2" fill="none" stroke="currentColour" viewBox="0 0 24 24">

  ...
  <div class="performance-attribution">
    ...
    <div v-else class="space-y-6">
      ...
      <div v-if="performanceData" class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <!-- Total Return -->
        <div class="bg-white rounded-lg shadow-md p-6">
          <p class="text-sm text-gray-600 mb-2">Total Return</p>
          <p class="text-3xl font-bold mb-1" :class="performanceData.total_return >= 0 ? 'text-green-600' : 'text-red-600'">
            {{ formatPercent(performanceData.total_return) }}
          </p>
          <p class="text-xs text-gray-500">{{ selectedPeriod.toUpperCase() }}</p>
        </div>

        <!-- Benchmark Return -->
        <div class="bg-white rounded-lg shadow-md p-6">
          <p class="text-sm text-gray-600 mb-2">Benchmark Return</p>
          <p class="text-3xl font-bold mb-1" :class="performanceData.benchmark_return >= 0 ? 'text-green-600' : 'text-red-600'">
            {{ formatPercent(performanceData.benchmark_return) }}
          </p>
          <p class="text-xs text-gray-500">{{ performanceData.benchmark_name || 'FTSE All-Share' }}</p>
        </div>

        <!-- Alpha (Excess Return) -->
        <div class="bg-white rounded-lg shadow-md p-6">
          <p class="text-sm text-gray-600 mb-2">Alpha (Excess Return)</p>
          <p class="text-3xl font-bold mb-1" :class="performanceData.alpha >= 0 ? 'text-green-600' : 'text-red-600'">
            {{ formatPercent(performanceData.alpha) }}
          </p>
          <p class="text-xs text-gray-500">vs Benchmark</p>
        </div>
      ...
      <div v-if="performanceData" class="bg-white rounded-lg shadow-md p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-6">Performance Attribution Breakdown</h3>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <!-- Attribution Chart -->
          <div>
            <h4 class="text-sm font-semibold text-gray-700 mb-3">Attribution by Source</h4>
            <apexchart
              type="bar"
      ...
      <div v-if="riskMetrics" class="bg-white rounded-lg shadow-md p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-6">Risk-Adjusted Performance Metrics</h3>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <!-- Beta -->
          <div class="border border-gray-200 rounded-lg p-4">
            <p class="text-sm text-gray-600 mb-2">Beta</p>
            <p class="text-2xl font-bold text-gray-800 mb-1">{{ formatDecimal(riskMetrics.beta) }}</p>
            <p class="text-xs text-gray-500">
          ...
          <!-- Max Drawdown -->
          <div class="border border-gray-200 rounded-lg p-4">
            <p class="text-sm text-gray-600 mb-2">Maximum Drawdown</p>
            <p class="text-2xl font-bold text-red-600 mb-1">{{ formatPercent(riskMetrics.max_drawdown) }}</p>
            <p class="text-xs text-gray-500">Largest peak-to-trough decline</p>
          </div>

          <!-- Volatility -->
          <div class="border border-gray-200 rounded-lg p-4">
            <p class="text-sm text-gray-600 mb-2">Volatility (Std Dev)</p>
            <p class="text-2xl font-bold text-gray-800 mb-1">{{ formatPercent(riskMetrics.volatility) }}</p>
            <p class="text-xs text-gray-500">Annualized</p>
          </div>

          <!-- Downside Deviation -->
          <div class="border border-gray-200 rounded-lg p-4">
            <p class="text-sm text-gray-600 mb-2">Downside Deviation</p>
            <p class="text-2xl font-bold text-gray-800 mb-1">{{ formatPercent(riskMetrics.downside_deviation) }}</p>
            <p class="text-xs text-gray-500">Below-target volatility</p>
          </div>

          <!-- Tracking Error -->
          <div class="border border-gray-200 rounded-lg p-4">
            <p class="text-sm text-gray-600 mb-2">Tracking Error</p>
            <p class="text-2xl font-bold text-gray-800 mb-1">{{ formatPercent(riskMetrics.tracking_error) }}</p>
            <p class="text-xs text-gray-500">Deviation from benchmark</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="performanceData && performanceData.asset_class_breakdown" class="bg-white rounded-lg shadow-md p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-6">Performance by Asset Class</h3>

        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-gray-200">
                <th class="text-left py-3 px-4 font-semibold text-gray-700">Asset Class</th>
                <th class="text-right py-3 px-4 font-semibold text-gray-700">Weight (%)</th>
                <th class="text-right py-3 px-4 font-semibold text-gray-700">Return (%)</th>
                <th class="text-right py-3 px-4 font-semibold text-gray-700">Contribution (%)</th>
                <th class="text-right py-3 px-4 font-semibold text-gray-700">vs Benchmark</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td class="text-right py-3 px-4" :class="assetClass.return >= 0 ? 'text-green-600' : 'text-red-600'">
                  {{ formatPercent(assetClass.return) }}
                </td>
                <td class="text-right py-3 px-4 font-semibold">
                  {{ formatPercent(assetClass.contribution) }}
                </td>
                <td class="text-right py-3 px-4" :class="assetClass.vs_benchmark >= 0 ? 'text-green-600' : 'text-red-600'">

    ...
    <div v-else class="space-y-6">
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div class="text-centre p-4 bg-amber-50 rounded-lg border border-amber-200">
      ...
      <div v-if="data.immediate && data.immediate.length > 0" class="bg-white border border-gray-200 rounded-lg p-5">
        <h5 class="text-md font-semibold text-red-800 mb-4 flex items-centre">
          <svg class="w-5 h-5 mr-2" fill="currentColour" viewBox="0 0 20 20">
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-start">
              <span class="flex-shrink-0 inline-flex items-centre justify-centre h-7 w-7 rounded-full bg-red-600 text-white text-sm font-bold mr-3">
                {{ index + 1 }}
              </span>
              <div class="flex-1">
                <p class="text-sm font-semibold text-gray-800 mb-1">{{ action.title }}</p>
                <p class="text-sm text-gray-600 mb-2">{{ action.description }}</p>
                <div class="flex items-centre gap-4 text-xs text-gray-500">
                  <span v-if="action.estimated_time">
                    <svg class="w-4 h-4 inline mr-1" fill="currentColour" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                    </svg>
      ...
      <div v-if="data.short_term && data.short_term.length > 0" class="bg-white border border-gray-200 rounded-lg p-5">
        <h5 class="text-md font-semibold text-amber-800 mb-4 flex items-centre">
          <svg class="w-5 h-5 mr-2" fill="currentColour" viewBox="0 0 20 20">
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-start">
              <span class="flex-shrink-0 inline-flex items-centre justify-centre h-7 w-7 rounded-full bg-amber-600 text-white text-sm font-bold mr-3">
                {{ index + 1 }}
              </span>
              <div class="flex-1">
                <p class="text-sm font-semibold text-gray-800 mb-1">{{ action.title }}</p>
      ...
      <div v-if="data.long_term && data.long_term.length > 0" class="bg-white border border-gray-200 rounded-lg p-5">
        <h5 class="text-md font-semibold text-blue-800 mb-4 flex items-centre">
          <svg class="w-5 h-5 mr-2" fill="currentColour" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
          </svg>
          Long-term Actions (12+ Months)
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-start">
              <span class="flex-shrink-0 inline-flex items-centre justify-centre h-7 w-7 rounded-full bg-blue-600 text-white text-sm font-bold mr-3">
                {{ index + 1 }}
              </span>
              <div class="flex-1">
                <p class="text-sm font-semibold text-gray-800 mb-1">{{ action.title }}</p>
      ...
      <div v-if="data.review_schedule" class="bg-blue-50 border border-blue-200 rounded-lg p-5">
        ...
        <div class="space-y-2">
          <div class="flex items-centre text-sm text-gray-700">
            <svg class="w-4 h-4 text-blue-600 mr-2" fill="currentColour" viewBox="0 0 20 20">
      ...
      <div class="bg-green-50 border border-green-200 rounded-lg p-5">
        <div class="flex items-start">
          <svg class="w-6 h-6 text-green-600 mr-3 flex-shrink-0 mt-1" fill="currentColour" viewBox="0 0 20 20">
